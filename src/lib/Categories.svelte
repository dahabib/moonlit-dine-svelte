...
<section class="fluid bg-yellow-600">
    <div class="shadow-xl shadow-neutral-100 border border-1 px-2">
        <div class="mx-auto container justify-center grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 py-6 gap-2 ">
            {#each categories as cat}
                <div class="bg-white rounded-lg p-10 items-center text-center bg-cover bg-center group md:lg:xl:border-r md:lg:xl:border-b hover:bg-slate-50 cursor-pointer" style="background-image: url({cat.strCategoryThumb})" on:click={ ()=> handleSelection(cat.strCategory)} data-sveltekit-preload-data="hover">
                    <p class="bg-lime-200 p-4 rounded-md text-2xl font-medium text-slate-800 mt-3">{cat.strCategory}</p>
                </div>
            {/each}
        </div>
    </div>
